...
inline fixed WaterBRDF (inout fixed3 Albedo, fixed Smoothness, fixed3 Specular, fixed wetFactor) {
	...
	// Tweak Specular Color towards Water / Water F0 specular is 0.02 (based on IOR of 1.33)
	Specular = lerp(Specular, 0.02, wetFactor);
	// Calc Roughness
	return lerp(1.0, Smoothness, lerp(1.0, factor, wetFactor) );
}
